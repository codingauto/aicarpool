...
# 
# 使用方法:
# 1. 默认安装（推荐普通用户）: curl -fsSL https://raw.githubusercontent.com/codingauto/aicarpool/main/scripts/deploy-centos.sh | bash
# 2. 强制root用户安装: FORCE_ROOT=true curl -fsSL https://raw.githubusercontent.com/codingauto/aicarpool/main/scripts/deploy-centos.sh | bash
# 3. 跳过网络检查: SKIP_NETWORK_CHECK=true curl -fsSL https://raw.githubusercontent.com/codingauto/aicarpool/main/scripts/deploy-centos.sh | bash
# 4. 完全非交互式: FORCE_ROOT=true SKIP_NETWORK_CHECK=true curl -fsSL https://raw.githubusercontent.com/codingauto/aicarpool/main/scripts/deploy-centos.sh | bash
