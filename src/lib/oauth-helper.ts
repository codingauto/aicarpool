/**
 * OAuth 辅助工具
 * 完整移植自 claude-relay-service 的 oauthHelper.js 实现
 */
 ...
 */
function generateAuthUrl(codeChallenge: string, state: string): string {
...
/**
 * 创建代理agent
 * @param {ProxyConfig|null} proxyConfig - 代理配置对象
 * @returns {object|null} 代理agent或null
 */
function createProxyAgent(proxyConfig: ProxyConfig | null) {
  if (!proxyConfig) {
    return null;
  }
 ...
 * @param {string} state - state 参数
 * @param {ProxyConfig|null} proxyConfig - 代理配置（可选）
 * @returns {Promise<TokenData>} Claude格式的token响应
 */
export async function exchangeCodeForTokens(
  ...
  try {
    ...
    if (error.response) {
      ...
      console.error('❌ OAuth token exchange failed with server error', {
        status: status,
      ...
      // 请求被发送但没有收到响应
      console.error('❌ OAuth token exchange failed with network error', {
      ...
      // 其他错误
      console.error('❌ OAuth token exchange failed with unknown error', {
        message: error.message,
        stack: error.stack
      });
 ...
 */
export function formatClaudeCredentials(tokenData: TokenData) {
  return {
    claudeAiOauth: {
...
export const CLAUDE_OAUTH_CONFIG = OAUTH_CONFIG;
export { generateState, generateCodeVerifier, generateCodeChallenge, generateAuthUrl, createProxyAgent };