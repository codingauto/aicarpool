...
export async function GET(
  ...
  try {
    ...
    if (process.env.NODE_ENV === 'development' && !token) {
      console.log('🔐 开发模式：Claude账号列表使用默认测试用户');
      user = {
        id: 'user_test_001',
    ...
    console.log(`🎯 API Claude账号: 返回企业 ${enterpriseId} 的 ${formattedAccounts.length} 个Claude账号`);

    return createApiResponse(formattedAccounts);

  } catch (error) {
    console.error('获取Claude账号失败:', error);
    return createApiResponse(false, null, '获取Claude账号失败', 500);
  }
}
